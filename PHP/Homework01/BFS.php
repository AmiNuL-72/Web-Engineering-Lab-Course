 ...
 *     c. search neighbors, if they haven't been visited,
 ...
 *
 * Implemented by enqueuing a path, instead of a node, for each neighbor.
 *
    ...
    while ($queue->count() > 0) {
        ...
        foreach ($graph[$node] as $neighbor) {
            if (!in_array($neighbor, $visited)) {
                ...
                # Build new path appending the neighbor then and enqueue it
                $new_path = $path;
                $new_path[] = $neighbor;
